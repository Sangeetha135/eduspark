...
import {
  ...
  generateQuiz,
  generatesubtitle,
} from "../helpers/helpingfunctions.js";
...
export const videoupload = async (req, res) => {
  try {
    ...
    const subtitlepath = path.join(
      __dirname,
      "uploads/transcription_timestamps.txt"
    );
    ...
    const video = new Video({
      ...
      summary:
        "tis id thensum,,m,asddmkmdnfoasmdnasjknfjknn,amnkjasksamkasmaskfas",
      questions: arr,
    });
    await video.save();
...
export const addcomment = async (req, res) => {
  ...
  await video.save();
  const video2 = await Video.findById(id).populate({
    path: "comments",
    populate: { path: "user" },
  });
...
export const putquery = async (req, res) => {
  const useridname = req.params.name;
...
export const setanswer = async (req, res) => {
  ...
  if (query) {
    query.answers.push({ username, isStudent, answer });
    await query.save();
    res.status(200).json(query);
  }
};
